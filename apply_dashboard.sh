...
#   bash apply_dashboard.sh
set -e

# ── 1. New Dashboard page (admin-only) ──────────────────────────────────────
mkdir -p src/app/dashboard
if [ -f src/app/dashboard/page.tsx ]; then
  echo "dashboard page: already exists — nothing to do."
else
...
EOF
  echo "Wrote src/app/dashboard/page.tsx"
fi

# ── 2. TopNav — admin-only "Dashboard" link ─────────────────────────────────
python3 - << 'PYEOF'
...
if "'/dashboard'" in content:
    print("TopNav.tsx: Dashboard link already present — nothing to do.")
else:
    old = "              {navLink('/library', 'Library')}\n              {navLink('/settings', 'Settings')}"
    new = "              {navLink('/dashboard', 'Dashboard')}\n              {navLink('/library', 'Library')}\n              {navLink('/settings', 'Settings')}"
    if old not in content:
        raise SystemExit("Expected nav link block not found in TopNav.tsx — aborting.")
    content = content.replace(old, new)
    with open(path, "w") as f:
        f.write(content)
    print("TopNav.tsx: added the admin-only Dashboard link.")
PYEOF

echo ""
echo "Restart your dev server, sign in as an admin, and check:"
echo "  1. 'Dashboard' now appears in the nav (admin only — sign in as a"
echo "     reviewer-role account if you want to confirm it's hidden for them)."
echo "  2. /dashboard loads: total matters, reviewed vs filed-only, average"
echo "     score, grade distribution bar chart, top flagged concerns, and a"
echo "     12-month volume chart."
echo "  3. Numbers look sane against what's actually in your Library."
echo ""
