  ...
  const issuesHtml = findings
    .map(
      ...
      <div style="border-left:4px solid ${sevColor[f.severity]};border:1px solid #D8D3C7;border-left-width:4px;padding:16px;margin-bottom:16px;background:#F7F5F1;">
        ...
        <span style="font-family:monospace;font-size:11px;color:#8C8777;text-transform:uppercase;margin-left:8px;">${escapeHtml(f.concernLabel)}</span>
        ...
        <p><strong>Why it matters:</strong> ${escapeHtml(f.analysis)}</p>
        <p><strong>Negotiation direction:</strong> ${escapeHtml(f.recommendation)}</p>
        ${
          redlines[f.uid]
            ? `<p><strong>Drafted redline:</strong></p><pre style="white-space:pre-wrap;background:#EFE9DC;padding:12px;font-family:monospace;font-size:12px;">${escapeHtml(redlines[f.uid])}</pre>`
            : ''
        }
      </div>`
    )
...
<body>
  ...
  <h1>${escapeHtml(contract.clientName)} — ${escapeHtml(contract.projectName)} (${escapeHtml(contract.projectNumber)})</h1>
  <div class="meta">
    ${escapeHtml(contract.docType)} · Counterparty: ${escapeHtml(contract.counterparty)} · Generated ${generatedAt.toLocaleString()}
  </div>
  <div class="summary">
