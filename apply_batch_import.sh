...
#   bash apply_batch_import.sh
set -e

# ── 1. New page: src/app/batch-import/page.tsx ──────────────────────────────
mkdir -p "src/app/batch-import"
cat > "src/app/batch-import/page.tsx" << 'VS_APPLY_EOF_page'
...
VS_APPLY_EOF_page
echo "Wrote src/app/batch-import/page.tsx"

# ── 2. New component: src/components/intake/BatchImportView.tsx ────────────
cat > "src/components/intake/BatchImportView.tsx" << 'VS_APPLY_EOF_batchview'
'use client';
...
VS_APPLY_EOF_batchview
echo "Wrote src/components/intake/BatchImportView.tsx"

# ── 3. Link it from the Library home ────────────────────────────────────────
python3 - << 'PYEOF'
...
if "/batch-import" in content:
    print("ClientListView.tsx: batch import link already present — nothing to do.")
else:
    old = """      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl text-ink">Client Library</h1>
        <Button variant="primary" onClick={() => setAdding((v) => !v)}>
          + New Client
        </Button>
      </div>"""
    new = """      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl text-ink">Client Library</h1>
        <div className="flex items-center gap-2">
    ...
    if old not in content:
        raise SystemExit(
            ...
            "src/components/library/ClientListView.tsx — aborting. Paste me "
            "the current file and I'll fix it by hand."
        )
    content = content.replace(old, new)
    with open(path, "w") as f:
        f.write(content)
    print("ClientListView.tsx: added Batch import link.")
PYEOF

echo ""
echo "Done. Restart your dev server and test:"
echo "  1. From the Library home, click 'Batch import'."
echo "  2. Set a default client, then select several files at once."
echo "  3. Confirm/edit job name, job number, and doc type per row (job"
echo "     name is pre-guessed from the file name — check it)."
echo "  4. Click 'Import all' and confirm each row shows 'Filed' and the"
echo "     files land in the right client's Drive folder."
echo ""
